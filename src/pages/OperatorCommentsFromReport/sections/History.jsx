...
import { AlertCircle, CalendarDays, ClipboardList, Loader2, RefreshCw, ChevronRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useListArchivedInspectionsMutation } from '../../../redux/services/inspectionsApi';
...
const History = ({ darkMode }) => {
    const [listArchivedInspections, { data: response, isLoading, isError }] = useListArchivedInspectionsMutation();
    const navigate = useNavigate();

    const loadArchivedInspections = useCallback(() => {
        listArchivedInspections();
    }, [listArchivedInspections]);
            ...
            <div className={`mx-auto rounded-2xl border ${panelClass}`}>
                <div className={`flex items-center justify-between border-b px-6 py-5 ${darkMode ? 'border-slate-700/60' : 'border-slate-200'}`}>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className={`text-xl font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>Inspection History</h1>
                            <p className={`mt-1 text-sm ${mutedTextClass}`}>Select an archived inspection to view its details.</p>
                        </div>
                    </div>

                    <button
                        type="button"
                        onClick={loadArchivedInspections}
                        disabled={isLoading}
                        aria-label="Refresh inspection history"
                        title="Refresh inspection history"
                ...
                <div className="p-6">
                    {isLoading && (
                        <div className={`flex min-h-40 flex-col items-center justify-center gap-3 ${mutedTextClass}`}>
                            ...
                            <p className="text-sm">Loading inspection history...</p>
                        </div>
                    )}
                    ...
                    {!isLoading && isError && (
                        <div className={`flex min-h-40 flex-col items-center justify-center gap-3 rounded-xl border border-dashed ${darkMode ? 'border-rose-400/30 text-rose-300' : 'border-rose-200 text-rose-600'}`}>
                            ...
                            <p className="text-sm">Unable to load inspection history.</p>
                            <button type="button" onClick={loadArchivedInspections} className="inline-flex items-center gap-2 rounded-lg bg-sky-500 px-3 py-2 text-sm font-semibold text-white hover:bg-sky-400">
                    ...
                    {!isLoading && !isError && inspections.length === 0 && (
                        <div className={`flex min-h-40 flex-col items-center justify-center gap-2 ${mutedTextClass}`}>
                            ...
                            <p className="text-sm">No archived inspections found.</p>
                        </div>
                    )}
                            ...
                            <div className={`divide-y ${darkMode ? 'divide-slate-700/60' : 'divide-slate-200'}`}>
                                {inspections.map((inspection) => (
                                    <button
                                        type="button"
                                        key={inspection.id}
                                        onClick={() => navigate(`${ROUTES.OPERATOR_COMMENTS_FROM_REPORT_INSPECTIONS}/${inspection.id}`, { state: { fromHistory: true } })}
                                    ...
                                    >
                                        <span className={`text-sm font-semibold ${darkMode ? 'text-slate-100' : 'text-slate-800'}`}>{getVesselLabel(inspection)}</span>
                                        ...
                                        <span className={`inline-flex items-center justify-end gap-1 text-xs font-semibold transition-colors ${darkMode ? 'text-sky-400 group-hover:text-sky-300' : 'text-sky-600 group-hover:text-sky-700'}`}>
                                            View <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                                        </span>
                                    </button>
                                ))}
                            </div>
